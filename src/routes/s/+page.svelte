...
<style>
	...
	.keywords-bg { position: fixed; inset: 0; display: flex; gap: 32px; padding: 40px 32px; pointer-events: none; user-select: none; z-index: 0; flex-wrap: wrap; align-content: center; justify-content: center; }
	...
	.card-glow { position: absolute; inset: 0; background: radial-gradient(ellipse at 50% 0%, var(--md-sys-color-primary-container) 0%, transparent 70%); pointer-events: none; opacity: 0.15; }
	.card-content { position: relative; padding: 40px 28px 24px; display: flex; flex-direction: column; align-items: center; }
	.spin-icon { font-size: 48px; color: var(--md-sys-color-primary); animation: spin 1s linear infinite; margin-bottom: 20px; }
	@keyframes spin { from { transform: rotate(0deg) } to { transform: rotate(360deg) } }
	.countdown { font-size: 16px; color: var(--md-sys-color-on-surface-variant); margin: 0 0 20px; text-align: center; font-weight: 400; letter-spacing: 0.3px; }
	...
	.skip :global(button) { display: inline-block; padding: 12px 36px; background: var(--md-sys-color-primary); color: var(--md-sys-color-on-primary); border: none; border-radius: 999px; font-size: 14px; font-weight: 500; cursor: pointer; letter-spacing: 0.3px; transition: all 0.2s ease; box-shadow: 0 4px 20px rgba(127,51,0,0.25); }
